        ...
        <div className="space-y-12 py-8">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 border-b border-border pb-8">
                <div>
                    ...
                    <p className="text-primary/70 text-sm">إضافة وتعديل بيانات الكتب والمؤلفات العلمية للشيخ.</p>
                </div>
                <button
                ...
                <div className="bg-card p-6 rounded-2xl border border-secondary/20 shadow-xl animate-in fade-in slide-in-from-top-4 duration-300">
                    ...
                    <form onSubmit={handleAddBook} className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div className="space-y-2 col-span-full">
                            <label className="text-sm font-bold text-primary/70 px-1">عنوان الكتاب</label>
                            <input
                                required
                                type="text"
                        ...
                        <div className="space-y-2 col-span-full">
                            <label className="text-sm font-bold text-primary/70 px-1">وصف الكتاب</label>
                            <textarea
                                required
                                rows={3}
                        ...
                        <div className="space-y-2">
                            <label className="text-sm font-bold text-primary/70 px-1">رابط صورة الغلاف</label>
                            <input
                                type="url"
                                placeholder="https://..."
                        ...
                        <div className="space-y-2">
                            <label className="text-sm font-bold text-primary/70 px-1">سنة النشر</label>
                            <input
                                type="number"
                                className="w-full px-4 py-3 rounded-xl border border-border focus:outline-none focus:ring-2 focus:ring-secondary/50 transition-all font-serif text-primary placeholder:text-primary/50 bg-background"
                        ...
                        <div className="space-y-2">
                            <label className="text-sm font-bold text-primary/70 px-1">رابط ملف الـ PDF (اختياري)</label>
                            <input
                                type="url"
                                placeholder="https://..."
                                className="w-full px-4 py-3 rounded-xl border border-border focus:outline-none focus:ring-2 focus:ring-secondary/50 transition-all font-serif text-left ltr text-primary placeholder:text-primary/50 bg-background"
            ...
            {editingBook && (
                <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
                    <div className="bg-card w-full max-w-2xl rounded-3xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200 text-right" dir="rtl">
                        ...
                        <form onSubmit={handleUpdateBook} className="p-6 space-y-4">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div className="space-y-2">
                                    <label className="text-sm font-bold text-primary/70 px-1">رابط الغلاف</label>
                                    <input
                                        type="url"
                            ...
                            <div className="space-y-2">
                                <label className="text-sm font-bold text-primary/70 px-1">رابط الـ PDF</label>
                                <input
                                    type="url"
                                    className="w-full px-4 py-3 rounded-xl border border-border focus:outline-none focus:ring-2 focus:ring-secondary/50 transition-all font-serif text-left ltr text-primary"
                            ...
                            <div className="flex gap-3 pt-4">
                                ...
                                <button
                                    ...
                                    className="px-6 bg-muted text-primary/70 py-3 rounded-xl font-bold hover:bg-muted/80 transition-all"
                                >
                                    إلغاء
                                </button>
                            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                {loading ? (
                    <div className="col-span-full py-20 text-center">
                        ...
                        <span className="text-primary/70 font-serif">جاري تحميل الكتب...</span>
                    </div>
                ) : books.length === 0 ? (
                    <div className="col-span-full py-20 text-center bg-card rounded-2xl border border-dashed border-border">
                        <span className="text-primary/70 font-serif">لا يوجد كتب مضافة بعد.</span>
                    </div>
                ) : (
                    books.filter(b => b.title.includes(searchTerm)).map((book) => (
                        <div key={book.id} className="bg-card p-5 rounded-2xl border border-border shadow-sm hover:shadow-md transition-all group flex flex-col sm:flex-row gap-6">
                            ...
                            <div className="flex flex-col flex-grow">
                                ...
                                <p className="text-xs text-primary/70 line-clamp-2 mb-4 leading-relaxed font-serif">
                                    {book.description}
                                </p>

                                <div className="mt-auto flex items-center justify-between border-t border-border pt-4">
                                    <span className="text-xs font-bold text-primary/70">{book.published_year} م</span>
